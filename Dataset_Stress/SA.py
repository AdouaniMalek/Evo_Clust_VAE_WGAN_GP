...
import json
from Evaluation_metrics import compute_epsilon_identifiability, compute_nndr, compute_cluster_statistical_parity
...
# Load the dataset
df = pd.read_csv(dataset_directory + "corporate_stress_dataset.csv")
...
# %%
df_raw = pd.read_csv(dataset_directory + "corporate_stress_dataset_preprocessed.csv")
...
def fairness_objective(X_syn, cluster_labels_syn, sensitive_attrs_syn):
    sp = compute_cluster_statistical_parity(
    cluster_labels_syn,
    sensitive_attrs_syn
)
...
def evaluate_point(theta):
    ...
    min_len = min(
    len(X_syn),
    len(cluster_labels_syn),
    len(sensitive_syn)
)

    X_syn = X_syn[:min_len]
    cluster_labels_syn = cluster_labels_syn[:min_len]
    sensitive_syn = sensitive_syn[:min_len]
...
EVAL_BUDGET = 80
N_SEEDS = 30
OUTPUT_FILE = "SA_results_D2.json"
...
def propose_neighbor(theta):
    ...
    if idx in [0, 3, 4]:
        ...
        theta_new[idx] = int(np.clip(theta[idx] + step, low, high))
    else:
        step_size = 0.1 * (high - low)
        theta_new[idx] = np.clip(theta[idx] + np.random.normal(0, step_size), low, high)

    return theta_new


# ---- SA CORE ----
def simulated_annealing(n_iters=EVAL_BUDGET, T0=1.0, alpha=0.99, seed=None):
    np.random.seed(seed)
    random.seed(seed)
    ...
    best_val = scalar_objective(theta)

    T = T0
    history = []

    for it in range(n_iters):
        theta_new = propose_neighbor(theta)
        val_new = scalar_objective(theta_new)

        delta = val_new - best_val
        accept = (delta < 0) or (random.random() < math.exp(-delta / max(T, 1e-8)))
        ...
        print(f"[SA][seed={seed}][iter={it+1}/{n_iters}] J={best_val:.4f}")

    return best_theta, best_val, history


# ---- SINGLE RUN ----
def run_SA_once(seed):
    theta, val, history = simulated_annealing(seed=seed)
    f_util, f_fair, f_priv = evaluate_point(theta)
...
def run_multiple_SA():
    ...
    for seed in range(N_SEEDS):
        print(f"\n[RUN][SA] seed {seed+1}/{N_SEEDS}")
        results.append(run_SA_once(seed))

    # ---- NORMALIZATION ----
    obj = np.array([r["objective"] for r in results])
    obj_norm = minmax_normalize(obj)

    for i, r in enumerate(results):
        r["objective_norm"] = float(obj_norm[i])
...
if __name__ == "__main__":
    SA_results = run_multiple_SA()
    save_results(SA_results)