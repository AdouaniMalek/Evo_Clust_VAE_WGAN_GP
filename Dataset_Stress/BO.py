...
EVAL_BUDGET = 80
N_SEEDS = 30
OUTPUT_FILE = "BO_results_D2.json"
...
def run_BO(seed):
    ...
    start = time.time()
    study.optimize(optuna_objective, n_trials=EVAL_BUDGET)
...
def run_multiple_BO():
    ...
    for seed in range(N_SEEDS):
        print(f"\n[RUN][BO] seed {seed+1}/{N_SEEDS}")
        results.append(run_BO(seed))
...
if __name__ == "__main__":
    BO_results = run_multiple_BO()
    save_results(BO_results)