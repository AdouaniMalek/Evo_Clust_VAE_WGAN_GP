...
train_protected_attributes =  df[['Gender','Ethnic_2.0', 'Ethnic_3.0', 'Ethnic_4.0']].to_numpy()
...
def train_wgan_fair(
    ...
    for epoch in range(num_epochs):
        for i_batch, (real_data, protected_attributes) in enumerate(dataloader):
            ...
            protected_dict = {
                "Gender": protected_attributes[:, 2].to(device),
                "Ethnicity": protected_attributes[:, 3].to(device),
...
def scalar_objective(theta, w=(1.0, 1.0, 1.0)):
    ...
    w_u, w_f, w_p = w
    return w_u * f_util + w_f * f_fair + w_p * f_priv
...
EVAL_BUDGET = 80
N_SEEDS = 30
OUTPUT_FILE = "SA_results_D1.json"
...
def propose_neighbor(theta):
    ...
    if idx in [0, 3, 4]:  # integer params
        step = random.randint(-2, 2)
        theta_new[idx] = int(np.clip(theta[idx] + step, low, high))
    else:  # continuous params
        step_size = 0.1 * (high - low)
        theta_new[idx] = float(np.clip(
            theta[idx] + np.random.normal(0, step_size),
            low, high
        ))
...
def simulated_annealing(n_iters=EVAL_BUDGET, T0=1.0, alpha=0.99, seed=None):
    ...
    current_val = scalar_objective(theta)
    ...
    for it in range(n_iters):
        ...
        # ---- FIX: compare to current, not best ----
        delta = val_new - current_val

        accept = (delta < 0) or (
            random.random() < math.exp(-delta / max(T, 1e-8))
        )
        ...
        print(f"[SA][seed={seed}][iter={it+1}/{n_iters}] best={best_val:.4f}")
...
def run_SA_once(seed):
    theta, best_val, history = simulated_annealing(seed=seed)
    f_util, f_fair, f_priv = evaluate_point(theta)
